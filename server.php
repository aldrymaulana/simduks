...
else
{
	...
	switch($oper)
	{
		case 'edit':
			$id = $_POST['id'];
			$sql = "update invheader set invdate ='$inv_date', amount = $amount,
				note = '$note', tax = $tax, total = $total where id = $id";
			$result =  $mysqli_connection->query($sql);
			check_error($mysqli_connection);			
			$sql = "select client_id from invheader where id=$id";			
			$result = $mysqli_connection->query($sql);
			$row = $result->fetch_object();
			if($row){
				$client_id = $row->client_id;
				$sql = "update clients set name ='$name' where client_id = $client_id";
